...
import { useUpdateQualityAcceptance } from '@/features/manage-quality-acceptance/model/useUpdateQualityAcceptance'
...
export default function QualityAcceptanceEditPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { data: qualityAcceptance, isLoading } = useQualityAcceptance(id)
    const updateMutation = useUpdateQualityAcceptance(id)
    ...
    useEffect(() => {
        if (qualityAcceptance) {
            setFormData({
                docNumber: qualityAcceptance.docNumber || '',
                docDate: qualityAcceptance.docDate || '',
                organizationId: qualityAcceptance.organizationId || '',
                warehouseId: qualityAcceptance.warehouseId || '',
            })
        }
    }, [qualityAcceptance])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await updateMutation.mutateAsync({
                ...
                lines: qualityAcceptance?.lines || []
            })
            toast.success(t('common.success'))
            router.push(`/inventory/quality-acceptance/${id}`)
        } catch (error: any) {
    ...
    if (!qualityAcceptance) return <div>{t('common.notFound')}</div>
    ...
    return (
        <div className="space-y-6">
            <GovBreadcrumb items={[
                ...
                { label: qualityAcceptance.docNumber || '', href: `/inventory/quality-acceptance/${id}` },
                { label: t('common.edit') }
            ]} />

            <GovCard>
                <GovCardHeader>
                    <GovCardTitle>{t('qualityAcceptances.editAcceptance')}</GovCardTitle>
                </GovCardHeader>
                <GovCardContent>
                    <form onSubmit={handleSubmit} className="space-y-4 max-w-2xl">
                        ...
                        <div>
                            ...
                            <p className="font-medium">
                                <span className={qualityAcceptance.status === 'DRAFT' ? 'text-gray-600' : qualityAcceptance.status === 'POSTED' ? 'text-green-600' : 'text-red-600'}>
                                    {qualityAcceptance.status ? t(`documents.${qualityAcceptance.status}`) : t('documents.DRAFT')}
                                </span>
                            </p>
                        </div>
