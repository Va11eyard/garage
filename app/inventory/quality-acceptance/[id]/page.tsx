...
export default function QualityAcceptanceViewPage({ params }: { params: { id: string } }) {
    const { t } = useTranslation()
    const { data: doc, isLoading } = useQualityAcceptance(params.id)
    ...
    return (
        <div className="gov-page-content space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="gov-title">{t('qualityAcceptances.title')} #{doc.documentNumber}</h1>
                <div className="flex gap-2">
                    {doc.status === 'DRAFT' && (
                        <Button asChild className="gov-button-primary">
                            <Link href={`/inventory/quality-acceptance/${params.id}/edit`}>{t('common.edit')}</Link>
                        </Button>
                    )}
