...
export function ItemDetails({ id }: { id: string }) {
    ...
    return (
        <div className="max-w-xl mx-auto space-y-4">
            ...
            <div>{t('items.itemGroup')}: {data.itemGroupName}</div>
            <div>{t('items.unitOfMeasure')}: {data.unitOfMeasureName}</div>
            <div>{t('items.description')}: {data.description ?? <span className="text-gov-text-secondary">—</span>}</div>
